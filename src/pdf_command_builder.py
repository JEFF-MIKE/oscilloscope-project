...
def parse_command_line_args(env_filepath: str = '.env'):
    ...
    parser = argparse.ArgumentParser(description="Extracts command information from a PDF file, and enters it into a JSON file.")
    parser.add_argument('--pdf-file-path', type=str, help="The PDF file to extract command information from.", default=os.environ['PDF_FILE_PATH'])
    parser.add_argument('--output-file-path', type=str, help="The JSON file to output the command information to.", default=os.environ['OUTPUT_FILE_PATH'])
...
def prcoess_command_details(detailed_commands_by_category: dict[str, dict], scope_queries: dict, scope_commands: dict, doc: pymupdf.Document, command_categories: list[str]) -> None:
                    ...
                    if focused_table[0] == COMMAND_TABLE_ROW:
                        ...
                        for command, query, return_description in focused_table[1:]:
                            ...
                            if command_details.has_variables_in_command or command_details.has_inline_variables_in_command:
                                ...
                                command_details.is_implemented = (not contains_unprocessed_parameter)
                            else:
                                command_details.is_implemented = True
                            detailed_commands_by_category[category].append(asdict(command_details))

def write_command_data_to_file(output_file_path: str, scope_queries: dict, scope_commands: dict, detailed_commands_by_category: dict[str, dict]):
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"The PDF file {args.pdf_file_path} could not be found. Check the path and try again.")
        exit(1)
    ...
    prcoess_command_details(detailed_commands_by_category, scope_queries, scope_commands, doc, command_categories)
    write_command_data_to_file(args.output_file_path, scope_queries, scope_commands, detailed_commands_by_category)
    print("Scope Queries")
